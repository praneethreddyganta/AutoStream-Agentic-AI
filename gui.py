...
st.set_page_config(
    page_title="AutoStream AI Agent - Sales & Support Dashboard",
    page_icon="⚡",
    layout="wide",
...
if not verify_env_key():
    ...
    st.info(
        "AutoStream Agent requires a valid `GOOGLE_API_KEY` to be set in your `.env` file.\n\n"
...
with st.sidebar:
    # Branding header
    st.markdown('<div class="brand-title">⚡ AutoStream AI</div>', unsafe_allow_html=True)
    ...
    with st.container(border=True):
        st.markdown(f"**Status:** {lead_collection_stage.upper()}")
    ...
    with st.container(border=True):
        ...
        st.markdown(f"**Knowledge Index:** `FAISS (Local)`")
